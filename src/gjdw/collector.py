                    ...
                    if not item.get("文件获取截止时间") and item.get("公告全文"):
                        deadline = extract_date_after_keyword(
                            ...
                            ["文件获取截止时间", "获取截止时间", "截止时间", "投标截止时间",
                             ...
                             "文件获取结束时间", "招标文件获取结束", "标书发售截止",
                             "采购文件获取", "采购文件发售", "应答截止", "响应截止",
                             "获取时间结束", "发售时间", "获取时间截止"]
                        )
                        if deadline:
            ...
            try:
                ...
                if full_text and len(full_text.strip()) > 200 and project_name in full_text:
                    ...
                    if deadline:
                        ...
                        # 正则兜底：在全文中搜索"截止/结束"等字样附近的日期时间
                        fallback_patterns = [
                            r"(?:截止|结束|截至)[^日期位]*?(\d{4}年\d{1,2}月\d{1,2}日\s*\d{1,2}时\d{1,2}分)",
                            r"(?:截止|结束|截至)[^日期位]*?(\d{4}年\d{1,2}月\d{1,2}日)",
                            r"(?:截止|结束|截至)[^日期位]*?(\d{4}-\d{1,2}-\d{1,2}\s+\d{1,2}:\d{1,2})",
                            r"(?:截止|结束|截至)[^日期位]*?(\d{4}-\d{1,2}-\d{1,2})",
                        ]
                        for fb_pattern in fallback_patterns:
                            fb_match = re.search(fb_pattern, full_text)
                            if fb_match:
                                fb_date = parse_date(fb_match.group(1))
                                if fb_date:
                                    result["文件获取截止时间"] = fb_date
                                    break
